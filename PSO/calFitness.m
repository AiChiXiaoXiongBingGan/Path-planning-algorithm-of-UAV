function particles = calFitness(startPos,goalPos,X,Y,Z,particles,d_ocean,d_air,h_ocean,radar,H_gun,air,biology)
for i = 1:size(particles,2)
    ...
    for j = 2:size(path,1)
        ...
        elseif (path(j,3) > h_ocean)&&(distance2 < H_gun(3)) % 避开高炮范围
            flag = 1;
            break
%         elseif (path(j,3) == (h_ocean*2))&&(x == air(1))&&(y == air(2)) % 避开大气威胁
%             flag = 1;
%             break 
%         elseif (path(j,3) == (h_ocean/2))&&(x == biology(1,1))&&(y == biology(1,2)) % 避开水下威胁
%             flag = 1;
%             break 
%         elseif (path(j,3) == (h_ocean/2))&&(x == biology(2,1))&&(y == biology(2,2)) 
%             flag = 1;
%             break 
%         elseif (path(j,3) == (h_ocean/2))&&(x == biology(3,1))&&(y == biology(3,2)) 
%             flag = 1;
%             break 
        end
    end  
